...
export function createOpenAICompatibleRequestOptions(
  sk,
  model,
  messages,
  options = {}
...
export function openAiCompatibleChat(
  ...
  const modelId = defaultModelIdResolver(model); // 取出模型ID
  if (!sk) {
    if (isBrowserExtension) {
      return onError(
        new Error(
          "Please configure the API key in the extension main page and reload this page"
        )
      );
    }
  ...
  fetch(`${fmtBaseUrl(baseUrl)}/chat/completions`, {
    ...createOpenAICompatibleRequestOptions(sk, modelId, messages, chatOptions),
      ...
      function read() {
        reader.read().then(({ done, value }) => {
          ...
          const content = deltas
            ...
            .filter(Boolean)
            .join("");
          const reasoning_content = deltas
...
export async function streamChat(
  ...
  if (isBoolean(enableThinking)) {
    ...
    modelChatOptions.thinking_budget = 8192;
  }
  try {
...
export function exportConfig() {
  ...
  a.href = url;
  a.download = "chatcols.config.json";
 ...
 * </Output>
 */
export function marcoLikeThought(
  content,
  tagThought = "Thought",
  tagOutput = "Output"
) {
