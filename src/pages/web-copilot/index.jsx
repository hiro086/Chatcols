...
export default function () {
  ...
  useEffect(() => {
    ...
    const handleMessage = (event) => {
      if (event.data && typeof event.data === "string") {
        ...
        if (message.type === "chatcols:web-copilot-init") {
          setDarkMode(message.isDarkTheme);
          setIsIFrame(message.isIFrame);
        }
        if (message.type === "chatcols:web-copilot-query") {
          setMessage(message);
          setTimeout(() => {
    ...
    const handleMouseMove = throttle((e) => {
      if (isDragging) {
        ...
        window.parent?.postMessage(
          JSON.stringify({
            type: "chatcols:web-copilot-move",
            position,
  ...
  return (
    <div className="flex flex-col w-full h-full">
      ...
      <div
        className="flex items-center w-full h-12 pl-4 pr-3"
      ...
      >
        ...
        <i
          ...
          onClick={() => {
            window.parent?.postMessage(
              JSON.stringify({ type: "chatcols:web-copilot-close" }),
              "*"
            );
          }}
        ></i>
