...
export const fetchUserInfo = async () => {
  ...
  const url = "https://aianswers.cn/v1/user/info";
  const options = {
    method: "GET",
    headers: {
...
export const getEnglishText = async (userInput, language) => {
  ...
  const result = await getChatCompletion(`${userInput}`, {
    systemPrompt: `You are a highly skilled translation engine with expertise in the technology sector. Your function is to translate texts accurately to ${targetLanguage}, maintaining the original format, technical terms, and abbreviations. Do not add any explanations or annotations to the translated text. Output translation directly without any additional text.`,
  });
  return result;
};
...
export const getWebSearchResults = async (userInput) => {
  const { zhipuai } = getJsonDataFromLocalStorage(
...
export const getResponseEvaluationResults = (
  userInput,
  systemPrompt,
  responses,
  judges = ["chatgpt-4o", "chatgpt-5", "chatgpt-4o-mini"]
) => {
  ...
  各模型回答如下：
  ${responses
    .map((item) => `<id>${item.model}</id><response>${item.content}</response>`)
    .join("\n")}
  ...
  return judges.map((judge) =>
    getChatCompletion(prompt, { modelId: judge, json: true, top_p: 0.2 })
      .then((res) => {
        try {
          console.log(res);
          let { best } = res;
...
export const checkNeedEnableThinking = async (modelId, content) => {
  ...
  const res = await getChatCompletion(
    `以下是一个用户输入，请你判断该输入是否需要模型进行“深度推理”。  
